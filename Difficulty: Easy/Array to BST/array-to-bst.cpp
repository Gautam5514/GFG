...
class Solution {
  public:
  Node *builtBSTinOrder(vector<int>& inorder, int start, int end) {
      if(start > end) return nullptr;
      int mid = (start + end) / 2;
      Node *node = new Node(inorder[mid]);
      node -> left = builtBSTinOrder(inorder, start, mid-1);
      node -> right = builtBSTinOrder(inorder, mid+1, end);
      return node;
  }
    Node* sortedArrayToBST(vector<int>& nums) {
        // Code here
        Node *root = builtBSTinOrder(nums, 0, nums.size()-1);
        return root;
    }
};
